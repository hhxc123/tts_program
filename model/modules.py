...
class VarianceAdaptor(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config, train_config):
        ...
        self.use_conv_embedding = self.learn_alignment \
                                  and (
                                              self.pitch_feature_level == "phoneme_level" or self.energy_feature_level == "phoneme_level")
    ...
    def binarize_attention_parallel(self, attn, in_lens, out_lens):
        ...
        with torch.no_grad():
            ...
            in_lens_cpu = in_lens.cpu().numpy()
            out_lens_cpu = out_lens.cpu().numpy()
            attn_cpu1 = attn_cpu.astype('float64')
            # in_lens_cpu1 = in_lens_cpu.astype('float64')
            # out_lens_cpu1 = out_lens_cpu.astype('float64')
            # print("the type of out_lens:", type(attn_cpu1))
            # attn_out = b_mas(attn_cpu1, in_lens_cpu1, out_lens_cpu1, width=1)
            attn_out = b_mas(attn_cpu, in_lens_cpu, out_lens_cpu, width=1)
...
class ConversationalContextEncoder(nn.Module):
    ...
    def forward(self, text_emb, speaker, history_text_emb, history_speaker, history_lens):
        ...
        # GRU
        enc_current = self.gru_linear(self.gru(enc_current)[0])  # 先经过gru, 再经过gru_linear
