# 这个文件的作用是把preprocessed_data/DailyTalk/val_*.txt中的内容合成语音
import sys
...
'''python synthesize.py --source preprocessed_data/DailyTalk/val_phone.txt \
--restore_step 900000 --mode batch --dataset DailyTalk'''
...
def synthesize(device, model, args, configs, vocoder, batchs, control_values):
    ...
    print("configs and control_values has loaded!")
    myvalue = 0
    for batch in batchs:
        myvalue += 1
        # print("batch信息:", batch)
        print("id:{0}正在执行,raw_texts是{1}".format(batch[0], batch[1]))
